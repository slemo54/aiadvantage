...
function injectHeadingIds(html: string): string {
  return html.replace(
    /<h([23])([^>]*)>(.*?)<\/h[23]>/gi,
    (_, level, attrs, text) => {
      const label = (text as string).replace(/<[^>]+>/g, "");
      const id = label
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "");
      ...
      <ul className="space-y-2">
        {items.map((item, index) => (
          <li key={item.id}>
            <a
              href={`#${item.id}`}
              className="flex items-start gap-2 text-sm text-zinc-400 transition-colors hover:text-[#22c55e]"
...
function RelatedArticles({ currentSlug }: { currentSlug: string }) {
  const related = SEED_ARTICLES.filter((a) => a.slug !== currentSlug).slice(
    0,
    3
  );
  return (
    <section className="border-t border-zinc-800 bg-zinc-950 py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          ...
          <Link
            href="/blog"
            className="flex items-center gap-1 text-sm font-medium text-[#22c55e] hover:text-[#4ade80]"
          >
          ...
          {related.map((article) => (
            ...
            >
              <article className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 transition-all hover:border-zinc-700">
                <div
                  className={`relative h-40 bg-gradient-to-br ${article.gradient}`}
                >
                  <div
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor: `${CATEGORIES[article.category as CategoryKey]?.accent ?? "#22c55e"}cc`,
                    }}
                  >
                    {CATEGORIES[article.category as CategoryKey]?.label ??
                      article.category}
                  </span>
                </div>
                <div className="p-4">
...
function ArticleNewsletter() {
  ...
  return (
    <section className="border-t border-zinc-800 py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center justify-between gap-6 rounded-2xl bg-gradient-to-r from-emerald-900 to-green-800 p-8 sm:flex-row sm:p-10">
...
export function ArticlePageClient({
  ...
  const category =
    CATEGORIES[article.category as CategoryKey] ?? CATEGORIES.ai_news;
    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <section className="relative">
        <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
          <div
            className={`relative aspect-[21/9] overflow-hidden rounded-2xl bg-gradient-to-br ${gradient}`}
          >
            {article.hero_image_url ? (
              ...
              <>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <svg
                    viewBox="0 0 200 200"
                    className="h-48 w-48 opacity-10"
                    fill="none"
                  >
                    <circle
                      cx="100"
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-12">
          ...
          <article className="lg:col-span-6">
            ...
            {/* Lead / description */}
            {article.meta_description && (
              <p className="mb-8 rounded-xl border-l-4 border-emerald-500 bg-emerald-950/30 px-5 py-4 text-base italic text-zinc-300">
                {article.meta_description}
              </p>
            )}
            ...
            <div className="mt-10 flex items-center justify-between border-t border-zinc-800 pt-6">
              <div className="flex items-center gap-3">
                <Button
                  variant="outline"
                  size="sm"
                  className="border-zinc-700 text-zinc-400 hover:border-[#22c55e] hover:text-[#22c55e]"
                >
            ...
            <div className="sticky top-24 space-y-6">
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
                ...
                <p className="mt-3 text-xs text-zinc-500">
                  Esploro l&apos;AI applicata al business e allo sviluppo
                  software.
                </p>
                <Link href="/about">
                  <Button
                    variant="outline"
                    size="sm"
                    className="mt-3 w-full border-zinc-700 text-xs text-zinc-400"
                  >
                    Profilo completo
                  </Button>
                </Link>
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
                ...
                <Link href="/#contact">
                  <Button
                    size="sm"
                    className="w-full bg-[#22c55e] text-xs text-black hover:bg-[#4ade80]"
                  >
                    Iscriviti gratis
                  </Button>
                </Link>
